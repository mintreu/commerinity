 ...
 * Team Page
 *
 * Shows direct referral members (first level)
...
const { team, teamMeta, isLoading, fetchTeam } = useNetwork()
...
onMounted(async () => {
  try {
    ...
    toast.add({
      title: 'Error',
      description: 'Failed to load referral members',
      color: 'error'
    })
  }
})
...
<template>
  <div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        ...
        <p class="text-gray-500 dark:text-gray-400">
          Direct referrals: <span class="font-semibold text-violet-600 dark:text-violet-400">{{ teamMeta?.total || 0 }}</span>
        </p>
      </div>
      <UButton
        to="/community"
    ...
    <template v-else-if="team.length > 0">
      ...
      <!-- Pagination -->
      <div
        v-if="teamMeta && teamMeta.last_page > 1"
        class="flex justify-center pt-4"
      >
        <UPagination
          :model-value="teamMeta.current_page"
          :total="teamMeta.total"
          :page-count="teamMeta.per_page"
    ...
    >
      ...
      >
        ...
        <div class="text-center">
          ...
          >
          <h4 class="mt-4 text-xl font-bold text-gray-900 dark:text-white">
            {{ selectedMember.name }}
          </h4>
        ...
        <div class="space-y-3">
          ...
          <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
            ...
            <p class="text-xs font-mono text-gray-900 dark:text-white">
              {{ selectedMember.uuid }}
            </p>
          </div>
        </div>
