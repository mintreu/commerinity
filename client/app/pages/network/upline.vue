...
<template>
  <div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">My Sponsor</h1>
        <p class="text-gray-500 dark:text-gray-400">Your direct upline/referrer</p>
      </div>
    ...
    <!-- Sponsor Card -->
    <div v-else-if="parent" class="glass-card overflow-hidden">
      <!-- Gradient Header -->
      <div class="bg-gradient-to-r from-violet-600 to-fuchsia-600 p-8 text-center">
        ...
        />
        <h2 class="mt-4 text-2xl font-bold text-white">{{ parent.name }}</h2>
        <UBadge color="white" variant="subtle" class="mt-2 capitalize">
      ...
      <div class="p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <div class="text-center p-4 bg-violet-50 dark:bg-violet-900/20 rounded-xl">
            <UIcon name="i-lucide-user" class="w-8 h-8 mx-auto text-violet-600 dark:text-violet-400 mb-2" />
            <p class="text-xs text-gray-500 dark:text-gray-400">Relationship</p>
            <p class="text-lg font-bold text-gray-900 dark:text-white">Sponsor</p>
          </div>

          <div class="text-center p-4 bg-fuchsia-50 dark:bg-fuchsia-900/20 rounded-xl">
            <UIcon name="i-lucide-award" class="w-8 h-8 mx-auto text-fuchsia-600 dark:text-fuchsia-400 mb-2" />
            <p class="text-xs text-gray-500 dark:text-gray-400">Their Level</p>
            <p class="text-lg font-bold text-gray-900 dark:text-white">{{ parent.level || 1 }}</p>
          </div>
        </div>
        ...
        <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
          ...
          <p class="text-sm font-mono text-gray-900 dark:text-white">{{ parent.uuid }}</p>
        </div>

        <!-- Note -->
        <div class="flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
          <UIcon name="i-lucide-info" class="w-5 h-5 text-blue-600 dark:text-blue-400 mt-0.5 shrink-0" />
          <div>
            <p class="text-sm text-blue-800 dark:text-blue-300 font-medium">Your Sponsor</p>
    ...
    <div v-else class="glass-card p-12 text-center">
      <div class="w-20 h-20 mx-auto bg-gradient-to-br from-violet-100 to-fuchsia-100 dark:from-violet-900/30 dark:to-fuchsia-900/30 rounded-full flex items-center justify-center mb-6">
        <UIcon name="i-lucide-crown" class="w-10 h-10 text-violet-600 dark:text-violet-400" />
      </div>
      <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">You're at the Top!</h3>
    ...
    <div class="glass-card p-6">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Your Referral Details</h3>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
          <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Your Referral Code</p>
          <p class="text-lg font-bold font-mono text-violet-600 dark:text-violet-400">{{ user?.referral_code }}</p>
        </div>
        <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
          <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Your Position</p>
          <p class="text-lg font-bold text-gray-900 dark:text-white capitalize">{{ user?.type || 'Member' }}</p>
        </div>
      </div>
    </div>
  </div>
