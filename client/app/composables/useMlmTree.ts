/**
 * Affiliate Tree Composable
 *
 * Handles fetching and managing Affiliate tree data for org chart visualization
 */
...
export function useAffiliateTree() {
  ...
  /**
   * Fetch Affiliate tree data for org chart
   */
  async function fetchTree(referralCode?: string): Promise<void> {
    ...
    try {
      const url = referralCode
        ? `${config.public.apiBase}/api/affiliate/tree?referral_code=${referralCode}`
        : `${config.public.apiBase}/api/affiliate/tree`
      ...
      console.error('Failed to fetch Affiliate tree:', err)
  ...
  async function fetchStats(): Promise<void> {
    try {
      const response = await useSanctumFetch(`${config.public.apiBase}/api/affiliate/stats`, {
        method: 'GET'
      })
