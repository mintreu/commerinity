...
const fetchPost = async () => {
  if (!slug.value) {
    status.value = 'error'
    errorMessage.value = 'Invalid content URL.'
    post.value = null
    related.value = []
    return
  }
  ...
  try {
    const response = await useSanctumFetch<ApiResourceResponse<PostDetail> | PostDetail>(`${baseUrl.value}/${slug.value}`)
    const resolvedPost = (response as ApiResourceResponse<PostDetail>)?.data ?? (response as PostDetail)
    if (!resolvedPost?.id) {
    ...
    useSeoMeta({
      title: resolvedPost?.seo_title || resolvedPost?.title || '',
      description: resolvedPost?.seo_description || resolvedPost?.excerpt || '',
      ogTitle: resolvedPost?.seo_title || resolvedPost?.title || '',
      ogDescription: resolvedPost?.seo_description || resolvedPost?.excerpt || '',
      ogImage: resolvedPost?.cover_image || undefined
    })
  } catch (err: unknown) {
    status.value = 'error'
  ...
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950">
    <section class="theme-gradient-brand text-white py-16">
      <div class="mx-auto max-w-4xl px-6 text-center space-y-4">
        ...
        <h1
          v-if="post"
          class="text-4xl md:text-5xl font-black leading-tight"
        >
          {{ post.title }}
        </h1>
        <p
          v-if="post?.excerpt"
          class="text-lg text-white/80 max-w-3xl mx-auto"
        >
          {{ post.excerpt }}
        </p>
      </div>
