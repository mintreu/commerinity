# Project Memory - Commerinity / Mintreu
...
- Docs of interest at root: `API_DOCUMENTATION.md`, `WALLET_IMPLEMENTATION_PLAN.md`, `WALLET_INTEGRATION_STATUS.md`.

## Backend stack (apiserver)
- Laravel 12, PHP 8.4 (per AGENTS.md); Sanctum v4; Livewire v3; Filament v4.
- Notable packages: Spatie activity log, Spatie media library (Filament plugin), Staudenmeir adjacency list, MoneyPHP, WebPush notifications, Flowframe Trends.
- Testing: Pest v4, PHPUnit v12; formatting via Pint.

## Frontend stack (client)
- Nuxt 4 + Nuxt UI + Tailwind CSS v4 + TypeScript.
- Pages include auth flows and dashboards per user type.
...
- Wallet system: balance/transactions, PIN + security questions, topups, withdrawals, payouts; amounts in paisa.
- E-commerce: catalog, cart, orders, wishlist, reviews; checkout flow via transaction UUID.
- Other domains: onboarding + KYC, subscriptions, notifications + push, helpdesk tickets, recruitment/careers, trends/analytics, ads, FAQ.
...
## Money + payments
- Monetary values are in paisa (1 INR = 100 paisa). Formatting via MoneyService.
- Cashfree used for wallet topup and payouts; webhooks update transaction status.

## Handy commands
- Backend: `php artisan serve`, `php artisan test`, `vendor/bin/pint`.
- Frontend: `npm run dev`, `npm run build`.

## Notes for future work
- Prefer Laravel conventions from `apiserver/AGENTS.md` (strict types, Pest tests, Pint).
- Use Eloquent relationships and avoid raw queries where possible.
