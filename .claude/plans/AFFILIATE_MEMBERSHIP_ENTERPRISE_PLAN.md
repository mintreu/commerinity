# Enterprise Affiliate & Membership System - Complete Architecture Plan

## Executive Summary

This plan outlines an **industry-standard, enterprise-grade Affiliate system** designed to compete with top Affiliate software providers like Infinite Affiliate, HybridAffiliate, and Prime Affiliate Software. The system supports multiple compensation plan types with a flexible, configurable architecture.

**Sources Referenced:**
- [HybridAffiliate - Affiliate Compensation Plans Compared](https://www.hybridaffiliate.io/blogs/affiliate-compensation-plans-compared-binary-matrix-and-unilevel-explained/)
- [Infinite Affiliate Software](https://infiniteaffiliatesoftware.com/)
- [Prime Affiliate Software](https://primeaffiliatesoftware.com/board-affiliate-plan/)

---

## Part 1: Industry Analysis & Plan Types

### 1.1 Affiliate Compensation Plan Types (Industry Standard)
...
│  │ (affiliate_plans)  │     │ (affiliate_ranks)  │     │                  │        │
│  └──────────────┘     └──────────────┘     └──────────────────┘        │
│         │                    │                                          │
│         ▼                    ▼                                          │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────────┐        │
│  │  PACKAGES    │────▶│    USERS     │────▶│  USER_PACKAGES   │        │
│  │(affiliate_packages)│     │  (members)   │     │                  │        │
│  └──────────────┘     └──────────────┘     └──────────────────┘        │
│                              │                                          │
│                              ▼                                          │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────────┐        │
│  │   NETWORK    │◀────│  GENEALOGY   │────▶│  COMMISSIONS     │        │
│  │ (affiliate_network)│     │(affiliate_genealogy│     │(affiliate_commissions) │        │
│  └──────────────┘     └──────────────┘     └──────────────────┘        │
│                                                                         │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────────┐        │
...
#### 2.2.1 `affiliate_plans` - Compensation Plan Configuration
...
#### 2.2.2 `affiliate_ranks` - Rank/Level Definitions

```php
Schema::create('affiliate_ranks', function (Blueprint $table) {
    $table->id();
    $table->uuid('uuid')->unique();
    $table->foreignId('plan_id')->constrained('affiliate_plans')->cascadeOnDelete();
...
#### 2.2.3 `affiliate_packages` - Subscription/Joining Packages
...
Schema::create('affiliate_packages', function (Blueprint $table) {
    ...
    $table->uuid('uuid')->unique();
    $table->foreignId('plan_id')->constrained('affiliate_plans')->cascadeOnDelete();
    $table->foreignId('entry_rank_id')->nullable()->constrained('affiliate_ranks')->nullOnDelete();
    ...
    // Upgrade Path
    $table->foreignId('upgrade_from_package_id')->nullable()->constrained('affiliate_packages')->nullOnDelete();
...
#### 2.2.4 `affiliate_bonus_types` - Configurable Bonus Types

```php
Schema::create('affiliate_bonus_types', function (Blueprint $table) {
    $table->id();
    $table->uuid('uuid')->unique();
...
#### 2.2.5 `affiliate_plan_bonus_configs` - Plan-specific Bonus Configuration

```php
Schema::create('affiliate_plan_bonus_configs', function (Blueprint $table) {
    $table->id();
    $table->foreignId('plan_id')->constrained('affiliate_plans')->cascadeOnDelete();
    $table->foreignId('bonus_type_id')->constrained('affiliate_bonus_types')->cascadeOnDelete();
...
#### 2.2.6 `affiliate_genealogy` - Network Tree Structure
...
Schema::create('affiliate_genealogy', function (Blueprint $table) {
    ...
    // Current Rank
    $table->foreignId('current_rank_id')->nullable()->constrained('affiliate_ranks')->nullOnDelete();
    $table->timestamp('rank_achieved_at')->nullable();
    $table->foreignId('highest_rank_id')->nullable()->constrained('affiliate_ranks')->nullOnDelete();
...
#### 2.2.7 `affiliate_user_packages` - User Package Subscriptions

```php
Schema::create('affiliate_user_packages', function (Blueprint $table) {
    $table->id();
    $table->uuid('uuid')->unique();

    $table->foreignId('user_id')->constrained()->cascadeOnDelete();
    $table->foreignId('package_id')->constrained('affiliate_packages')->cascadeOnDelete();
    $table->foreignId('genealogy_id')->constrained('affiliate_genealogy')->cascadeOnDelete();
    ...
    // Upgrade tracking
    $table->foreignId('upgraded_from_id')->nullable()->constrained('affiliate_user_packages')->nullOnDelete();
    $table->foreignId('upgraded_to_id')->nullable()->constrained('affiliate_user_packages')->nullOnDelete();
...
#### 2.2.8 `affiliate_commissions` - Commission Ledger
...
Schema::create('affiliate_commissions', function (Blueprint $table) {
    ...
    $table->foreignId('from_package_id')->nullable()->constrained('affiliate_user_packages')->nullOnDelete();
...
#### 2.2.9 `affiliate_rank_history` - Rank Achievement History

```php
Schema::create('affiliate_rank_history', function (Blueprint $table) {
    $table->id();
    $table->uuid('uuid')->unique();

    $table->foreignId('user_id')->constrained()->cascadeOnDelete();
    $table->foreignId('genealogy_id')->constrained('affiliate_genealogy')->cascadeOnDelete();
    $table->foreignId('rank_id')->constrained('affiliate_ranks')->cascadeOnDelete();
    $table->foreignId('previous_rank_id')->nullable()->constrained('affiliate_ranks')->nullOnDelete();
...
#### 2.2.10 `affiliate_payout_batches` - Batch Payout Processing
...
#### 2.2.11 `affiliate_pool_funds` - Global Bonus Pool

```php
Schema::create('affiliate_pool_funds', function (Blueprint $table) {
    $table->id();
    $table->uuid('uuid')->unique();
...
|-----------|-----------|--------|
| `stages` | `affiliate_plans` | More flexible, supports multiple plan types |
| `levels` | `affiliate_ranks` | Proper rank system with qualification criteria |
| `user_subscriptions` | `affiliate_user_packages` | Better package tracking |

### 5.3 New Tables to Create

1. `affiliate_plans` - Plan configuration
2. `affiliate_ranks` - Rank definitions
3. `affiliate_packages` - Package definitions
4. `affiliate_bonus_types` - Bonus type registry
5. `affiliate_plan_bonus_configs` - Plan-bonus mapping
6. `affiliate_genealogy` - Network tree
7. `affiliate_user_packages` - User subscriptions
8. `affiliate_commissions` - Commission ledger
9. `affiliate_rank_history` - Rank changes
10. `affiliate_payout_batches` - Batch payouts
11. `affiliate_pool_funds` - Global pools

### 5.4 Migration Order

```
1. affiliate_plans (no dependencies)
2. affiliate_ranks (depends on affiliate_plans)
3. affiliate_packages (depends on affiliate_plans, affiliate_ranks)
4. affiliate_bonus_types (no dependencies)
5. affiliate_plan_bonus_configs (depends on affiliate_plans, affiliate_bonus_types)
6. affiliate_genealogy (depends on users, affiliate_plans, affiliate_ranks)
7. affiliate_user_packages (depends on users, affiliate_packages, affiliate_genealogy, transactions)
8. affiliate_commissions (depends on users, affiliate_genealogy, affiliate_user_packages, affiliate_bonus_types)
9. affiliate_rank_history (depends on users, affiliate_genealogy, affiliate_ranks)
10. affiliate_payout_batches (depends on users)
11. affiliate_pool_funds (depends on affiliate_plans)
12. Update affiliate_commissions to add payout_batch_id FK
```
