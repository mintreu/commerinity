...
describe('Mode 2: Originator User Journey (Advisor)', function () {
    it('creates user with originator tracking', function () {
        ...
        $joinedUser = User::factory()->create([
            'name' => 'Joined User',
            'parent_id' => null, // Still root (no Affiliate sponsor)
    ...
    it('generates originator commission when user subscribes', function () {
        ...
        // User originated by advisor (no Affiliate sponsor)
        $joinedUser = User::factory()->create([
            'parent_id' => null,
            'originator_id' => $advisor->id,
        ]);
        ...
        // Note: Depends on affiliate config having originator commissions enabled
...
// =============================================================================
// FULL Affiliate JOURNEY: TEAM BUILDING & LEVEL PROGRESSION
// =============================================================================

describe('Full Affiliate Journey: Team Building', function () {
    it('builds 4-level deep team with commission flow', function () {
        ...
        $founderCommissions = AffiliateCommission::where('user_id', $founder->id)->get();
        ...
        $allCommissions = AffiliateCommission::all();

        // Founder should have commissions from Level 1 (direct) and Level 2 (indirect)
        $founderTotal = AffiliateCommission::where('user_id', $founder->id)->sum('gross_amount');
    ...
    it('propagates sales to upline genealogy', function () {
        ...
        $grandparentGenealogy = AffiliateGenealogy::forUser($grandparent->id);
        $parentGenealogy = AffiliateGenealogy::forUser($parent->id);
...
describe('Commission Calculations', function () {
    ...
    it('applies TDS deduction on high earnings', function () {
        ...
        $sponsorCommissions = AffiliateCommission::where('user_id', $sponsor->id)->get();
...
describe('Complete User Journey Simulation', function () {
    it('simulates full journey from registration to Diamond level', function () {
        ...
        $totalCommissions = AffiliateCommission::where('user_id', $user->id)->sum('gross_amount');
    ...
    it('simulates originator (advisor) journey with multiple originated users', function () {
        ...
        for ($i = 1; $i <= 5; $i++) {
            $user = User::factory()->create([
                ...
                'parent_id' => null, // Root users (no Affiliate sponsor)
                'originator_id' => $advisor->id,
                'originator_type' => 'advisor',
            ]);
        ...
        // 4. Advisor's Affiliate stats should still show 0 direct referrals
        // (Originated users are NOT in advisor's Affiliate downline)
        $advisorGenealogy = AffiliateGenealogy::forUser($advisor->id);
...
describe('Edge Cases', function () {
    ...
    it('prevents duplicate commission for same trigger', function () {
        ...
        $firstCount = AffiliateCommission::where('user_id', $sponsor->id)->count();
        ...
        $secondCount = AffiliateCommission::where('user_id', $sponsor->id)->count();
