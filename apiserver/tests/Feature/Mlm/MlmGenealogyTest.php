...
describe('MlmGenealogy Model', function () {

    it('can be created with factory', function () {
        $user = User::factory()->create();
        $genealogy = MlmGenealogy::factory()->forUser($user)->create();

        expect($genealogy)->toBeInstanceOf(MlmGenealogy::class)
    ...
    it('can be created with team counts', function () {
        $user = User::factory()->create();
        $genealogy = MlmGenealogy::factory()
            ->forUser($user)
    ...
    it('can set placement parent', function () {
        ...
        $parentGenealogy = MlmGenealogy::factory()->forUser($parent)->create();
        $childGenealogy = MlmGenealogy::factory()
            ->forUser($child)
    ...
    it('can get upline chain', function () {
        ...
        MlmGenealogy::factory()->forUser($grandparent)->atDepth(0)->create();
        MlmGenealogy::factory()->forUser($parent)->atDepth(1)->create();
        $genealogy = MlmGenealogy::factory()->forUser($user)->atDepth(2)->create();
    ...
    it('limits upline to max levels', function () {
        ...
        $lastGenealogy = MlmGenealogy::forUser($users[4]->id);
    ...
    it('can be found by user id', function () {
        ...
        $found = MlmGenealogy::forUser($user->id);
    ...
    it('returns null for non-existent user', function () {
        $found = MlmGenealogy::forUser(99999);
    ...
    it('can create for user with depth calculation', function () {
        ...
        MlmGenealogy::factory()->forUser($parent)->atDepth(0)->create();
        $childGenealogy = MlmGenealogy::createForUser($child->id);
    ...
    it('scope active works', function () {
        ...
        MlmGenealogy::factory()->forUser($user1)->create(['is_active' => true]);
        MlmGenealogy::factory()->forUser($user2)->create(['is_active' => false]);

        expect(MlmGenealogy::active()->count())->toBe(1);
    });
});
