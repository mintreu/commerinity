...
describe('MlmCommission Model', function () {

    it('can be created with factory', function () {
        $user = User::factory()->create();
        $commission = MlmCommission::factory()->forUser($user)->create();

        expect($commission)->toBeInstanceOf(MlmCommission::class)
    ...
    it('calculates net amount from gross minus deductions', function () {
        $user = User::factory()->create();
        $commission = MlmCommission::factory()
            ->forUser($user)
    ...
    it('can have genealogy relationship', function () {
        $user = User::factory()->create();
        $genealogy = MlmGenealogy::factory()->forUser($user)->create();

        $commission = MlmCommission::factory()
            ->withGenealogy($genealogy)
            ->create();

        expect($commission->genealogy)->toBeInstanceOf(MlmGenealogy::class)
    ...
    it('calculates total earnings for user', function () {
        ...
        $total = MlmCommission::getTotalEarnings($user->id);
    ...
    it('calculates pending earnings for user', function () {
        ...
        $pending = MlmCommission::getPendingEarnings($user->id);
    ...
    it('scope forUser works', function () {
        ...
        MlmCommission::factory()->forUser($user1)->count(3)->create();
        MlmCommission::factory()->forUser($user2)->count(2)->create();

        expect(MlmCommission::forUser($user1->id)->count())->toBe(3);
    });

    it('scope ofType works', function () {
        $user = User::factory()->create();

        MlmCommission::factory()->forUser($user)->sponsorBonus()->count(2)->create();
        MlmCommission::factory()->forUser($user)->levelCommission()->count(3)->create();

        expect(MlmCommission::ofType(CommissionTypeCast::SPONSOR_BONUS)->count())->toBe(2);
    });

    it('scope forPeriod works', function () {
        $user = User::factory()->create();

        MlmCommission::factory()->forUser($user)->thisMonth()->count(2)->create();
        MlmCommission::factory()->forUser($user)->lastMonth()->count(1)->create();

        expect(MlmCommission::forPeriod(now()->format('Y-m'))->count())->toBe(2);
    });
