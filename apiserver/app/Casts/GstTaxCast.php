...
enum GstTaxCast: string implements HasLabel
{
    ...
    {
        $formattedAmount = MoneyService::formatStatic($amount);
        ...
        if (str_starts_with($this->value, 'cs')) {
            ...
            return new HtmlString($formattedAmount.' <small>(CGST: '.MoneyService::formatStatic($halfTax).', SGST: '.MoneyService::formatStatic($halfTax).')</small>');
        }
