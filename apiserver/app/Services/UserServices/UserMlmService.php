...
/**
 * UserMlmService - Handles MLM network operations with optimized queries
 *
 * Responsibilities:
...
final class UserMlmService implements \App\Contracts\Services\UserMlmServiceInterface
{
    ...
    public function __construct()
    {
        ...
        $this->maxDepthLevels = config('mlm.matrix.max_depth_levels', 4);
    }
     ...
     */
    public function placeUser(User $newUser, ?User $sponsor = null, ?User $originator = null): MlmGenealogy
    {
        return DB::transaction(function () use ($newUser, $sponsor, $originator) {
            ...
            if ($sponsor) {
                ...
                // Calculate depth
                $parentGenealogy = MlmGenealogy::forUser($sponsor->id);
    ...
    public function distributeCommissions(int $userId, int $amountInPaisa, $commissionable): Collection
    {
        ...
        $rates = config('mlm.commission_rates', [
            1 => 10.0, // Level 1: 10%
            2 => 5.0,  // Level 2: 5%
            3 => 3.0,  // Level 3: 3%
            4 => 2.0,  // Level 4: 2%
        ...
        DB::transaction(function () use ($uplines, $amountInPaisa, $commissionable, $rates, &$commissions, $userId) {
            foreach ($uplines as $level => $uplineUser) {
                ...
                $uplineGenealogy = MlmGenealogy::forUser($uplineUser->id);
     ...
     */
    public function calculateOriginatorCommission(User $user, int $amountInPaisa, $commissionable): ?MlmCommission
    {
        if (! $user->originator) {
            return null;
        }
        ...
        $rate = config('mlm.originator_commission_rate', 5.0);
        $commissionAmount = (int) round($amountInPaisa * ($rate / 100));

        // Find originator's genealogy (if they have one)
        $originatorGenealogy = MlmGenealogy::forUser($originator->id);
    ...
    {
        $newMember = MlmGenealogy::forUser($newUserId);
        if (! $newMember) {
            return;
        }
