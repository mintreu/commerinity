...
/**
 * MLM Configuration Service
 *
 * Centralized service for checking MLM feature flags and configuration.
 * All commission enable/disable checks should go through this service.
 */
final class MlmConfigService
{
    // ========================================
    // MASTER SWITCHES
    // ========================================
    ...
    {
        return (bool) config('mlm.member_commissions.enabled', true);
    }

    public function isOriginatorCommissionsEnabled(): bool
    {
        return (bool) config('mlm.originator_commissions.enabled', true);
    }

    public function isTaskCommissionsEnabled(): bool
    {
        return (bool) config('mlm.task_commissions.enabled', true);
    }

    public function isIncomeDeductionEnabled(): bool
    {
        return (bool) config('mlm.income_deduction.enabled', false);
    }

    public function isAgentSalaryEnabled(): bool
    {
        return (bool) config('mlm.agent_salary.enabled', false);
    }

    public function isLevelFeaturesEnabled(): bool
    {
        return (bool) config('mlm.level_features.enabled', true);
    }
    ...
    public function getOriginatorJoiningConfig(): array
    {
        return [
            ...
            'type' => config('mlm.originator_commissions.joining_commission.type', 'percent'),
            'value' => (float) config('mlm.originator_commissions.joining_commission.value', 5),
        ];
    }
    ...
    public function getOriginatorRecurringConfig(): array
    {
        return [
            ...
            'type' => config('mlm.originator_commissions.recurring_commission.type', 'percent'),
            'value' => (float) config('mlm.originator_commissions.recurring_commission.value', 2),
            'frequency' => config('mlm.originator_commissions.recurring_commission.frequency', 'on_withdrawal'),
        ];
    }
    ...
    public function getIncomeDeductionConfig(): array
    {
        return [
            ...
            'percent' => (float) config('mlm.income_deduction.percent', 3),
            'description' => config('mlm.income_deduction.description', 'Platform Service Fee'),
            'show_on_invoice' => (bool) config('mlm.income_deduction.show_on_invoice', true),
        ];
    }
    ...
    public function getAgentSalaryTiers(): array
    {
        ...
        return config('mlm.agent_salary.tiers', []);
    }
    ...
    {
        return (bool) config('mlm.tds.enabled', true);
    }
    ...
    public function getTdsConfig(): array
    {
        return [
            ...
            'threshold_monthly' => (int) config('mlm.tds.threshold_monthly', 500000),
            'rate_percent' => (float) config('mlm.tds.rate_percent', 10),
        ];
    }
    ...
    public function calculateTds(int $amount, int $monthlyTotal): int
    {
        ...
        $threshold = (int) config('mlm.tds.threshold_monthly', 500000);
        $rate = (float) config('mlm.tds.rate_percent', 10);
    ...
    {
        return (bool) config('mlm.admin_fee.enabled', false);
    }
    ...
    public function getAdminFeeConfig(): array
    {
        return [
            ...
            'type' => config('mlm.admin_fee.type', 'percent'),
            'value' => (float) config('mlm.admin_fee.value', 0),
            'description' => config('mlm.admin_fee.description', 'Admin Fee'),
        ];
    }
    ...
    {
        return (bool) config('mlm.platform_fee.enabled', false);
    }
    ...
    public function getPlatformFeeConfig(?string $userType = null): array
    {
        $default = [
            ...
            'type' => config('mlm.platform_fee.default.type', 'percent'),
            'value' => (float) config('mlm.platform_fee.default.value', 2),
            'triggers' => config('mlm.platform_fee.triggers', []),
            'min_threshold' => (int) config('mlm.platform_fee.min_amount_threshold', 10000),
            'show_on_invoice' => (bool) config('mlm.platform_fee.show_on_invoice', true),
            'description' => config('mlm.platform_fee.description', 'Platform Service Fee'),
        ];
        ...
        $excludedTypes = config('mlm.platform_fee.excluded_types', []);
        ...
        $userConfig = config("mlm.platform_fee.user_types.{$userType}", []);
    ...
    public function getPlatformFeeSummary(): array
    {
        return [
            'enabled' => $this->isPlatformFeeEnabled(),
            'default' => config('mlm.platform_fee.default', []),
            'triggers' => config('mlm.platform_fee.triggers', []),
            'user_types' => collect(config('mlm.platform_fee.user_types', []))
                ->map(fn ($config, $type) => $this->getPlatformFeeConfig($type))
                ->toArray(),
            'excluded_types' => config('mlm.platform_fee.excluded_types', []),
            'min_threshold' => (int) config('mlm.platform_fee.min_amount_threshold', 0),
        ];
    }
    ...
    {
        $config = config("mlm.dashboard_visibility.{$userType}", []);
    ...
    public function getFeaturesAtRank(int $globalRank): array
    {
        ...
        $unlocks = config('mlm.level_features.unlocks', []);
        $features = [];
    ...
    /**
     * Resolve dynamic config values (e.g., 'config:mlm.income_deduction.enabled')
     */
