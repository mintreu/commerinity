...
use App\Events\Affiliate\CommissionsCalculated;
use App\Events\Affiliate\CommissionTriggered;
use App\Jobs\Affiliate\CalculateCommissionsJob;
use App\Jobs\Affiliate\ProcessCommissionJob;
use App\Models\Affiliate\AffiliateCommission;
use App\Services\Affiliate\Calculators\Affiliate\LevelCommissionCalculator;
use App\Services\Affiliate\Calculators\Affiliate\OriginatorJoiningCalculator;
use App\Services\Affiliate\Calculators\Affiliate\OriginatorRecurringCalculator;
use App\Services\Affiliate\Calculators\Affiliate\SponsorBonusCalculator;
use App\Services\Affiliate\Calculators\Task\TaskCompletionCalculator;
 ...
 *
 * Central service for processing Affiliate commissions. Follows these patterns:
...
final class CommissionProcessorService
{
    ...
    public function __construct(?AffiliateConfigService $configService = null)
    {
        $this->configService = $configService ?? new AffiliateConfigService;
    ...
    {
        Log::channel('affiliate')->info('CommissionProcessor: Dispatching async job', [
    ...
    public function calculate(CommissionTrigger $trigger): Collection
    {
        ...
        Log::channel('affiliate')->info('CommissionProcessor: Starting calculation', [
            'trigger_type' => $trigger->getTriggerType(),
            'trigger_id' => $trigger->getId(),
        ...
        foreach ($this->calculators as $calculator) {
            ...
            try {
                ...
                Log::channel('affiliate')->debug('Calculator executed', [
                ...
                Log::channel('affiliate')->error('Calculator failed', [
                    'calculator' => $calculator::class,
                    'error' => $e->getMessage(),
                    'trigger_id' => $trigger->getId(),
                ]);
        ...
        Log::channel('affiliate')->info('CommissionProcessor: Calculation complete', [
            'trigger_id' => $trigger->getId(),
    ...
    {
        $commissions = collect();
        $batchSize = (int) config('affiliate.batch_size', 50);
            ...
            DB::transaction(function () use ($batch, &$commissions, $monthlyTotals) {
                foreach ($batch as $result) {
                    ...
                    if ($this->isDuplicate($result)) {
                        Log::channel('affiliate')->warning('Duplicate commission skipped', [
        ...
        Log::channel('affiliate')->info('CommissionProcessor: Persistence complete', [
    ...
    private function preloadMonthlyTotals(array $userIds): array
    {
        ...
        $cacheKey = 'affiliate_monthly_totals_'.implode('_', $userIds).'_'.now()->format('Y_m');
    ...
    {
        $cacheKey = "affiliate_monthly_total_{$userId}_".now()->format('Y_m');
    ...
    private function isDuplicate(CommissionResult $result): bool
    {
        ...
        if ($idempotencyKey) {
            $existsByKey = AffiliateCommission::query()
    ...
    {
        $query = AffiliateCommission::query()->where('user_id', $userId);
