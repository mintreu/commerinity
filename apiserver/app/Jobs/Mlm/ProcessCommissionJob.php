...
final class ProcessCommissionJob implements ShouldQueue
{
     ...
     * Execute the job.
     */
    public function handle(MlmConfigService $configService): void
    {
        if ($this->batch()?->cancelled()) {
            return;
        }

        Log::channel('mlm')->info('ProcessCommissionJob started', [
        ...
        try {
            $commission = DB::transaction(function () use ($configService) {
                ...
                if ($this->isDuplicateCommission($result)) {
                    Log::channel('mlm')->warning('Duplicate commission detected, skipping', [
            ...
            if ($commission) {
                ...
                Log::channel('mlm')->info('ProcessCommissionJob completed', [
            ...
            Log::channel('mlm')->error('ProcessCommissionJob failed', [
                'recipient_id' => $this->result->recipientId,
                'type' => $this->result->type,
                'error' => $e->getMessage(),
            ]);
    ...
    private function applyDeductionsIfNeeded(
        ...
        MlmConfigService $configService,
    ): CommissionResult {
     ...
     */
    private function creditUserWallet(MlmCommission $commission): void
    {
        ...
        //     'type' => 'commission',
        //     'reference_type' => MlmCommission::class,
        //     'reference_id' => $commission->id,
        //     'description' => $commission->description,
        // ]);

        Log::channel('mlm')->info('Wallet credit pending implementation', [
            'commission_id' => $commission->id,
    ...
    {
        Log::channel('mlm')->critical('ProcessCommissionJob permanently failed', [
            'recipient_id' => $this->result->recipientId,
            'type' => $this->result->type,
            'gross_amount' => $this->result->grossAmount,
