...
use App\Models\User;
use App\Notifications\Affiliate\CommissionProcessingCompletedNotification;
...
final class BatchCommissionProcessingJob implements ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(CommissionProcessorService $processor): void
    {
        ...
        Log::channel('affiliate')->info('BatchCommissionProcessingJob started', [
            'process_type' => $this->processType,
        ...
        try {
            ...
            Log::channel('affiliate')->info('BatchCommissionProcessingJob completed', $stats);
            ...
            Log::channel('affiliate')->error('BatchCommissionProcessingJob failed', [
                ...$stats,
    ...
    {
        if (! config('affiliate.originator_commissions.recurring_commission.enabled', false)) {
            return;
        }

        // This would be implemented based on withdrawal triggers or monthly income
        Log::channel('affiliate')->info('Processing originator recurring commissions');
    }
    ...
    {
        if (! config('affiliate.member_commissions.matching_bonus.enabled', false)) {
            return;
        }

        Log::channel('affiliate')->info('Processing matching bonuses');
    ...
    {
        Log::channel('affiliate')->info('Processing monthly settlement');
    ...
    {
        if (! config('affiliate.agent_salary.enabled', false)) {
            return;
        }

        Log::channel('affiliate')->info('Processing agent salaries');
    ...
    {
        if (! config('affiliate.member_commissions.pool_bonus.enabled', false)) {
            return;
        }

        Log::channel('affiliate')->info('Processing pool bonuses');
    ...
    {
        Log::channel('affiliate')->critical('BatchCommissionProcessingJob permanently failed', [
            'process_type' => $this->processType,
