...
final class CalculateCommissionsJob implements ShouldQueue
{
    ...
    public function handle(CommissionProcessorService $processor): void
    {
        ...
        Log::channel('affiliate')->info('CalculateCommissionsJob started', [
        ...
        try {
            if ($this->persistImmediately) {
                ...
                Log::channel('affiliate')->info('CalculateCommissionsJob completed with persistence', [
                    'trigger_id' => $this->trigger->getId(),
                ...
                Log::channel('affiliate')->info('CalculateCommissionsJob completed (calculation only)', [
                    'trigger_id' => $this->trigger->getId(),
            ...
            Log::channel('affiliate')->error('CalculateCommissionsJob failed', [
                'trigger_id' => $this->trigger->getId(),
                'error' => $e->getMessage(),
    ...
    {
        Log::channel('affiliate')->critical('CalculateCommissionsJob permanently failed', [
            'trigger_type' => $this->trigger->getTriggerType(),
            'trigger_id' => $this->trigger->getId(),
