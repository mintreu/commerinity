...
/**
 * MLM Commission System Configuration
 *
 * This config controls ALL MLM commission features.
...
return [
    ...
    */
    'matrix' => [
        'max_direct_children' => (int) env('MLM_MAX_DIRECT_CHILDREN', 5),
        'max_depth' => (int) env('MLM_MAX_DEPTH', 4),
    ],
    ...
    */
    ...
    | the member into the system (separate from MLM tree)
    ...
    */
    'originator_commissions' => [
        ...
        'enabled' => (bool) env('MLM_ORIGINATOR_COMMISSIONS_ENABLED', true),

        // Joining Commission - originator gets % when originated user subscribes
        'joining_commission' => [
            'enabled' => (bool) env('MLM_ORIGINATOR_JOINING_ENABLED', true),
            'type' => env('MLM_ORIGINATOR_JOINING_TYPE', 'percent'), // 'percent' or 'fixed'
            'value' => (float) env('MLM_ORIGINATOR_JOINING_VALUE', 5), // 5% of subscription
        ...
        'recurring_commission' => [
            'enabled' => (bool) env('MLM_ORIGINATOR_RECURRING_ENABLED', false),
            'type' => env('MLM_ORIGINATOR_RECURRING_TYPE', 'percent'),
            'value' => (float) env('MLM_ORIGINATOR_RECURRING_VALUE', 2), // 2% of withdrawal
            'frequency' => env('MLM_ORIGINATOR_RECURRING_FREQ', 'on_withdrawal'), // 'on_withdrawal' or 'monthly'
        ],
    ],
    ...
    */
    'income_deduction' => [
        ...
        'enabled' => (bool) env('MLM_INCOME_DEDUCTION_ENABLED', false),

        // Deduction percentage from member commission earnings
        'percent' => (float) env('MLM_INCOME_DEDUCTION_PERCENT', 3), // 3% cut
        ...
        'description' => env('MLM_INCOME_DEDUCTION_DESC', 'Platform Service Fee'),

        // Show on member invoice/dashboard
        'show_on_invoice' => (bool) env('MLM_INCOME_DEDUCTION_SHOW_INVOICE', true),
    ],
    ...
    */
    'agent_salary' => [
        ...
        'enabled' => (bool) env('MLM_AGENT_SALARY_ENABLED', false),
        ...
        'payout_frequency' => env('MLM_SALARY_FREQUENCY', 'monthly'), // 'weekly', 'monthly'

        // Day of month for salary payout (1-28)
        'payout_day' => (int) env('MLM_SALARY_PAYOUT_DAY', 1),
    ],
    ...
    */
    'task_commissions' => [
        ...
        'enabled' => (bool) env('MLM_TASK_COMMISSIONS_ENABLED', true),

        // Task completion commission
        'task_completion' => [
            'enabled' => (bool) env('MLM_TASK_COMPLETION_ENABLED', true),
            // Task definitions stored in database (mlm_tasks table)
            // Each task has: name, description, reward_type, reward_value, criteria
        ],

        // Milestone/Goal achievement bonus
        'milestone_bonus' => [
            'enabled' => (bool) env('MLM_MILESTONE_BONUS_ENABLED', true),
            // Milestones defined in database (mlm_milestones table)
            // Examples: First 10 referrals, ₹1 lakh team sales, etc.
        ],

        // Referral bonus (non-MLM, simple referral)
        'referral_bonus' => [
            'enabled' => (bool) env('MLM_REFERRAL_BONUS_ENABLED', true),
            'type' => env('MLM_REFERRAL_BONUS_TYPE', 'fixed'), // 'percent' or 'fixed'
            'value' => (int) env('MLM_REFERRAL_BONUS_VALUE', 100_00), // ₹100 in paisa
            // Different from sponsor_bonus - this is for non-member referrals
        ],

        // Performance/KPI bonus
        'performance_bonus' => [
            'enabled' => (bool) env('MLM_PERFORMANCE_BONUS_ENABLED', false),
            // KPIs defined in database (mlm_kpis table)
            // Examples: Monthly sales target, customer retention rate
        ],

        // Custom commission type (fully database-driven)
        'custom' => [
            'enabled' => (bool) env('MLM_CUSTOM_COMMISSION_ENABLED', true),
            // All config comes from database (mlm_commission_types table)
    ...
    */
    'level_features' => [
        'enabled' => (bool) env('MLM_LEVEL_FEATURES_ENABLED', true),
    ...
    */
    'tds' => [
        'enabled' => (bool) env('MLM_TDS_ENABLED', true),
        'threshold_monthly' => (int) env('MLM_TDS_THRESHOLD', 500000), // 5000 INR in paisa
        'rate_percent' => (float) env('MLM_TDS_RATE', 10), // 10%
    ],
    ...
    | - member: Regular MLM members
    | - advisor/agent: Originators who recruit
    | - promoter: Hybrid users (both MLM + originator)
    | - staff: Future support
    |
    | Excluded: draft users, regular users (non-MLM)
    */
    'platform_fee' => [
        // Master switch
        'enabled' => (bool) env('MLM_PLATFORM_FEE_ENABLED', false),

        // Default fee for all applicable user types
        'default' => [
            'type' => env('MLM_PLATFORM_FEE_TYPE', 'percent'), // 'percent' or 'fixed'
            'value' => (float) env('MLM_PLATFORM_FEE_VALUE', 2), // 2% default
        ],

        // When to apply the fee
        'triggers' => [
            'on_withdrawal' => (bool) env('MLM_PLATFORM_FEE_ON_WITHDRAWAL', true),
            'on_commission' => (bool) env('MLM_PLATFORM_FEE_ON_COMMISSION', false),
            'monthly' => (bool) env('MLM_PLATFORM_FEE_MONTHLY', false),
        ],

        // Per user type configuration (overrides default when set)
        'user_types' => [
            'member' => [
                'enabled' => (bool) env('MLM_PLATFORM_FEE_MEMBER_ENABLED', true),
                'type' => env('MLM_PLATFORM_FEE_MEMBER_TYPE', null), // null = use default
                'value' => env('MLM_PLATFORM_FEE_MEMBER_VALUE', null),
            ],
            'advisor' => [
                'enabled' => (bool) env('MLM_PLATFORM_FEE_ADVISOR_ENABLED', true),
                'type' => env('MLM_PLATFORM_FEE_ADVISOR_TYPE', null),
                'value' => env('MLM_PLATFORM_FEE_ADVISOR_VALUE', null),
            ],
            'agent' => [
                'enabled' => (bool) env('MLM_PLATFORM_FEE_AGENT_ENABLED', true),
                'type' => env('MLM_PLATFORM_FEE_AGENT_TYPE', null),
                'value' => env('MLM_PLATFORM_FEE_AGENT_VALUE', null),
            ],
            'promoter' => [
                'enabled' => (bool) env('MLM_PLATFORM_FEE_PROMOTER_ENABLED', true),
                'type' => env('MLM_PLATFORM_FEE_PROMOTER_TYPE', null),
                'value' => env('MLM_PLATFORM_FEE_PROMOTER_VALUE', null),
            ],
            'staff' => [
                'enabled' => (bool) env('MLM_PLATFORM_FEE_STAFF_ENABLED', false),
                'type' => env('MLM_PLATFORM_FEE_STAFF_TYPE', null),
                'value' => env('MLM_PLATFORM_FEE_STAFF_VALUE', null),
            ],
        ],
        ...
        'min_amount_threshold' => (int) env('MLM_PLATFORM_FEE_MIN_AMOUNT', 10000), // Rs 100

        // Show on user invoice/statement
        'show_on_invoice' => (bool) env('MLM_PLATFORM_FEE_SHOW_INVOICE', true),

        // Description shown to user
        'description' => env('MLM_PLATFORM_FEE_DESC', 'Platform Service Fee'),

        // Where the collected fees go
        'collection_wallet' => env('MLM_PLATFORM_FEE_WALLET', 'platform_fees'),
    ],
    ...
    */
    'admin_fee' => [
        'enabled' => (bool) env('MLM_ADMIN_FEE_ENABLED', false),
        'type' => env('MLM_ADMIN_FEE_TYPE', 'percent'),
        'value' => (float) env('MLM_ADMIN_FEE_VALUE', 0),
        'description' => env('MLM_ADMIN_FEE_DESC', 'Admin Fee'),
    ],
    ...
    |--------------------------------------------------------------------------
    */
    'default_sponsor_bonus_percent' => (float) env('MLM_DEFAULT_SPONSOR_BONUS', 10),
    'default_level_rates' => [
        1 => (float) env('MLM_DEFAULT_LEVEL_1_RATE', 5),
        2 => (float) env('MLM_DEFAULT_LEVEL_2_RATE', 3),
        3 => (float) env('MLM_DEFAULT_LEVEL_3_RATE', 2),
        4 => (float) env('MLM_DEFAULT_LEVEL_4_RATE', 1),
    ],
    ...
    */
    'dashboard_visibility' => [
        ...
        'advisor' => [
            ...
            'show_originator_earnings' => 'config:mlm.originator_commissions.enabled',
            'show_salary_progress' => 'config:mlm.agent_salary.enabled',
            'show_targets' => 'config:mlm.agent_salary.enabled',
        ],

        // What promoter users see (hybrid - both MLM tree + can originate)
        'promoter' => [
            'show_mlm_earnings' => true,
            'show_team_tree' => true,
            'show_originated_users' => true,
    ...
    */
    'invoice' => [
        ...
        'show_commission_breakdown' => (bool) env('MLM_INVOICE_SHOW_BREAKDOWN', true),

        // Show deduction line item (dynamic based on income_deduction.enabled)
        'show_deductions' => 'config:mlm.income_deduction.enabled',

        // Show originator cut (dynamic based on originator_commissions.enabled)
        'show_originator_cut' => 'config:mlm.originator_commissions.enabled',
    ],
];
