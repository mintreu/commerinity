...
const fetchPage = async () => {
  ...
  try {
    // Determine URL dynamically based on route
    const pathParts = route.path.split('/').filter(Boolean)
    // If first part is "pages", strip it for nested pages
    ...
    if (!res || !res.data?.data) {
      // API returned nothing → show error section
...
// -------------------- Watch route changes --------------------
onMounted(fetchPage)
watch(route, fetchPage, { immediate: true })
  ...
  <div class="min-h-screen flex flex-col gap-1  mt-14 ">
    ...
    <section v-else-if="error" class="h-full grow flex flex-col justify-center items-center  text-center ">
      <div class="my-10">
        ...
        <nuxt-link to="/" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded">Go Home</nuxt-link>
      </div>
    </section>
